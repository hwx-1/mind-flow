...
<div align="center">
  ...
  <p>
    ...
    <img src="https://img.shields.io/badge/HarmonyOS-6.1.0(23)-1B6FFF?style=flat-square&logo=huawei" alt="HarmonyOS">
    <img src="https://img.shields.io/badge/ArkTS-ArkUI-0A59F7?style=flat-square" alt="ArkTS">
  </p>
  <p><strong>三端同源：🦋 鸿蒙原生 App（ArkTS/ArkUI） · 🌐 React 网页端 · ☕ Spring Boot 后端</strong></p>
</div>
...
* **三端同源、鸿蒙原生**：在 React 网页端之外，另提供基于 **HarmonyOS（ArkTS/ArkUI）** 的原生移动 App（适配手机/平板），与网页端共用同一套后端 API 与账号体系；移动端独有 **全局自定义 RGB 主题换肤**、图片九宫格与全屏手势缩放查看；并支持 **进入弹窗系统公告**（管理后台统一发布，多端共享）。
...
* **鸿蒙原生移动端**：`ArkTS` + `ArkUI` 声明式开发，`HarmonyOS 6.1.0(23)` 适配手机/平板；网络层基于官方 `RemoteCommunicationKit (rcp)` 与 `NetworkKit (WebSocket)`；状态与持久化采用 `AppStorage`/`PersistentStorage` 配合 `@State`/`@Prop`/`@Link`/`@StorageProp` 装饰器；并调用 `LocationKit`（定位）、`SensorServiceKit`（振动）、`MediaLibraryKit`（相册）等系统能力。
...
├── [系统公告]
│   ├── 管理后台统一发布 / 下线（多端共享同一数据源）
│   ├── 进入应用弹窗展示最新公告
│   └── 「我知道了」/「不再显示（1 天）」交互
└── [鸿蒙端专属]
    ├── 全局自定义 RGB 主题换肤（响应式、可持久化）
    ├── 图片九宫格缩略图 + 全屏查看器（滑动 / 捏合 / 双击缩放）
    └── 调用 GPS 定位、相册、振动等系统能力
```
...
### 3.3 鸿蒙移动端目录结构
```text
MentalHealth/                       # DevEco Studio 工程（ArkTS / ArkUI）
└── entry/src/main/
    ├── ets/
    │   ├── pages/Index.ets         # 主框架（Tabs：社区 / AI / 消息 / 我的）
    │   ├── view/                   # 页面与组件（按业务垂直划分）
    │   │   ├── community/          # 社区：信息流、详情、发布、搜索、定位选择
    │   │   ├── ai/                 # AI：首页、对话、历史
    │   │   ├── message/            # 即时通讯：会话列表、聊天室、好友
    │   │   ├── mine/               # 我的：资料、设置、主题色、我的内容
    │   │   ├── auth/               # 登录 / 注册
    │   │   └── common/             # 帖子卡片、图片网格、全屏查看器、公告弹窗
    │   ├── api/                    # HttpClient(rcp) · Api(接口集合) · WsClient(长连接)
    │   ├── store/                  # Auth(登录态) · AppTheme(主题) · AnnouncementStore(公告)
    │   ├── utils/                  # UploadUtil · LocationUtil · TimeUtil ...
    │   ├── model/Models.ets        # TS 接口/数据模型
    │   └── constants/Const.ets     # 服务器地址等全局常量
    ├── resources/                  # 资源（颜色、媒体、profile、network_config）
    └── module.json5                # 模块与权限声明（INTERNET/LOCATION/VIBRATE 等）
```
...
-- 15. 系统公告表 (移动端 / 网页端进入弹窗共享)
CREATE TABLE `announcement` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '公告ID',
  `title` varchar(200) DEFAULT '' COMMENT '标题',
  `content` text NOT NULL COMMENT '公告内容',
  `status` tinyint NOT NULL DEFAULT '1' COMMENT '1:上线 / 0:下线(逻辑删除)',
...
## 📱 7. 鸿蒙移动端 (HarmonyOS App) 设计与实现

心晴在 React 网页端之外，提供基于 **HarmonyOS** 的原生移动 App（工程 `MentalHealth/`）。该端采用 **ArkTS** 语言与 **ArkUI** 声明式开发范式，与网页端共用同一套后端 API 与账号体系，覆盖社区、AI 咨询、即时通讯、个人中心与系统公告等全部核心场景，并适配 **手机与平板** 两类设备；工程约 7000 行 ArkTS 代码、31 个页面与组件、41 个接口方法。

### 7.1 移动端分层架构

```mermaid
flowchart TB
    subgraph 表现层["表现层 (ArkUI 声明式)"]
      P["Index 主框架(Tabs)"] --- V["社区 / AI / 消息 / 我的 / 认证 / 公共 组件"]
    end
    subgraph 状态层["状态与持久化层"]
      ST["AppStorage / PersistentStorage<br/>@State · @Prop · @Link · @StorageProp<br/>Auth · AppTheme · AnnouncementStore"]
    end
    subgraph 业务层["业务与工具层"]
      B["HttpClient(rcp) · WsClient · UploadUtil · LocationUtil · TimeUtil"]
    end
    subgraph 能力层["系统能力层 (HarmonyOS Kits)"]
      K["RemoteCommunicationKit · NetworkKit · LocationKit · SensorServiceKit · MediaLibraryKit · CoreFileKit"]
    end
    BE["后端服务 (HTTPS / WSS)"]
    表现层 --> 状态层 --> 业务层 --> 能力层 --> BE
```
...
**（1）网络统一封装 HttpClient（基于官方 rcp）**：统一处理基址拼接、令牌注入、`R<T>` 解包、401 重登与失败重试。

```typescript
// 发起一次请求并解包后端统一返回结构
private async execOnce<T>(method: HttpMethod, url: string, body?: object): Promise<T> {
  const session = rcp.createSession({ requestConfiguration: this.config });
  try {
    const req = new rcp.Request(url, method, this.buildHeaders(), body);
    const resp = await session.fetch(req);              // 发送请求
    if (resp.statusCode === 401) {                       // 令牌失效→清登录态并跳登录
      await Auth.clear();
      router.replaceUrl({ url: 'view/auth/LoginPage' });
      throw new Error('unauthorized');
    }
    const json = JSON.parse(resp.toString() ?? '') as ApiResult<T>;
    if (json.code !== 0) throw new Error('business_error'); // 业务错误
    return json.data;                                    // 仅返回 data 主体
  } finally {
    session.close();                                     // 释放会话
  }
}
...
**（2）即时消息长连接 WsClient（握手带 JWT + 心跳 + 指数退避重连）**：保障实时私聊与通知稳定。

```typescript
connect(): void {
  const token = Auth.getToken();
  if (token.length === 0 || this.connected || this.ws !== null) return; // 防重复连接
  this.ws = webSocket.createWebSocket();
  this.ws.on('open', () => { this.connected = true; this.startHeartbeat(); }); // 心跳保活
  this.ws.on('message', (_e: BusinessError, data: string | ArrayBuffer) => {
    if (typeof data !== 'string') return;
    const obj = JSON.parse(data) as WsEnvelope;          // 解析服务端事件
    if (obj.event) this.handlers.forEach((h) => h(obj.event!, obj.data ?? {}));
  });
  this.ws.on('close', () => this.handleDrop());          // 断线→重连
  this.ws.on('error', () => this.handleDrop());
  // 握手时通过查询参数携带 JWT，供后端鉴权
  this.ws.connect(`${Const.WS_URL}?token=${token}`, (err: BusinessError) => { if (err) this.handleDrop(); });
}

// 断线后退避重连：退避时间随失败次数指数增长，上限 30s
private scheduleReconnect(): void {
  if (this.reconnectTimer >= 0 || Auth.getToken().length === 0) return;
  const delay = Math.min(BASE_DELAY * Math.pow(2, this.reconnectAttempts), MAX_DELAY);
  this.reconnectAttempts += 1;
  this.reconnectTimer = setTimeout(() => { this.reconnectTimer = -1; this.connect(); }, delay);
}
```

**（3）图片上传（沙箱拷贝 + rcp 多部分表单）**：rcp 无法直接读取相册 URI，需先拷贝至沙箱；且必须设置 `remoteFileName`，否则后端取不到文件名后缀而判为不支持的格式。
...
const form = new rcp.MultipartForm({
  'file': {
    ...
    // 关键：携带文件名，否则后端 getOriginalFilename() 取不到后缀→上传失败
    remoteFileName: file.fileName
  }
});
const request = new rcp.Request(`${Const.BASE_URL}/upload/${kind}`, 'POST', headers, form);
const response = await session.fetch(request);           // 提交并等待响应
```

**（4）全屏图片查看器手势缩放（ZoomableImage）**：Swiper 承载多图，子组件支持捏合/双击放大、拖动；放大态下禁用翻页避免冲突。
...
.gesture(                                                // 双指捏合缩放
  PinchGesture({ fingers: 2 })
    ...
    .onActionUpdate((e: GestureEvent) => {
      this.scaleValue = this.clampScale(this.baseScale * e.scale); // 限制 1~4 倍
    })
    .onActionEnd(() => { if (this.scaleValue <= 1) this.resetTransform(); else this.syncZoom(); })
)
.parallelGesture(                                        // 与 Swiper 并行的拖动
  PanGesture().onActionUpdate((e: GestureEvent) => {
    if (this.scaleValue > 1) { this.offsetX = this.baseX + e.offsetX; this.offsetY = this.baseY + e.offsetY; }
  })
)
```

**（5）全局自定义 RGB 主题换肤（AppTheme + @StorageProp）**：主色持久化于 `PersistentStorage` 并镜像到 `AppStorage`，各页面通过 `@StorageProp` 订阅，实现“一处修改、全局换肤”。

```typescript
// 设置主色（#RRGGBB），自动持久化并刷新浅色衬底；订阅页面随之实时重渲染
static setColor(hex: string): void {
  const norm: string = AppTheme.normalize(hex);
  AppStorage.setOrCreate<string>(KEY_PRIMARY, norm);            // 已 persistProp，自动持久化
  AppStorage.setOrCreate<string>(KEY_PRIMARY_LIGHT, AppTheme.lightOf(norm)); // 推导浅色衬底
}
// 页面侧：订阅即可，颜色变化自动刷新
@StorageProp('themePrimary') themePrimary: string = '#D98E73';
```

**（6）进入弹窗系统公告（AnnouncementStore + CustomDialog）**：启动后拉取最新公告，支持「我知道了」（下次仍弹）与「不再显示」（按公告 id 屏蔽 1 天，持久化）。

```typescript
// 进入主页后拉取最新公告并按屏蔽规则决定是否弹出
private loadAnnouncement(): void {
  AnnouncementApi.latest().then((a: AnnouncementVo | null) => {
    if (a && a.content.length > 0 && AnnouncementStore.shouldShow(a.id)) this.showAnnouncement(a);
  }).catch(() => {});
}
// 「不再显示」：记录公告 id 与截止时间(now+24h)
static dismissForDay(id: number): void {
  AppStorage.setOrCreate<number>(KEY_ID, id);
  AppStorage.setOrCreate<number>(KEY_UNTIL, Date.now() + 24 * 60 * 60 * 1000);
}
```

### 7.3 移动端构建与运行

1. 使用 **DevEco Studio** 打开 `MentalHealth/` 工程并完成 SDK / 签名配置。
2. 修改服务器地址 `entry/src/main/ets/constants/Const.ets`：
   ...
   > 真机/模拟器本地联调可临时改为 `http://<服务器IP>/api` 与 `ws://<服务器IP>/api/ws/im`，并在 `network_config.json` 放行明文域名；正式上架须使用 HTTPS。
3. 连接真机或模拟器，点击 **Run** 构建安装运行。

---

## 🔌 8. RESTful API 接口规范拓扑 (API Docs Snapshot)
...
## 🚀 9. 多环境全场景部署指南 (Deployment Guide)

### 9.1 方式一：Docker Compose 现代化容器编排一键启动
...
### 9.3 极其关键：Nginx 核心全量配置文件
> 鸿蒙 App 无需服务端部署，使用 **DevEco Studio** 打包后安装至真机即可；仅需将 `Const.ets` 中的 `BASE_URL`/`WS_URL` 指向线上域名（HTTPS/WSS）。
...
## 🧩 10. 踩坑与故障排查记录 (Troubleshooting)

下表汇总项目联调与部署过程中遇到的典型问题及其根因与解决方案，供复现与排障参考：

| 现象 | 根因 | 解决 |
| --- | --- | --- |
| 鸿蒙真机无法连接服务器（`osErr:113`，连 443 失败） | 服务器仅开放 80 端口，而 App 走 HTTPS(443) | 为域名配置 SSL 证书 + 放行安全组 443 端口 |
| 消息不实时（NETSTACK `wsi is nullptr`） | Nginx 反代缺少 WebSocket 协议升级头 | `/api/ws/` 增加 `Upgrade` 与 `Connection "upgrade"` |
| 真机图片上传失败（提示不支持的格式） | rcp 多部分表单未携带文件名 | `MultipartForm` 字段设置 `remoteFileName` |
| 发帖图片一直「加载中」 | ArkUI `ForEach` 以下标为 key，列表项不随数据刷新 | key 改为包含上传状态（如 `index-状态-url`） |
| 自定义主题色无法全局生效 | 资源色为编译期常量，运行时不可变 | 引入响应式主题仓库 `AppTheme`，页面用 `@StorageProp` 订阅 |
| 后端部署后接口仍是旧逻辑 | systemd 运行的 jar 文件名/路径与上传的不一致 | 覆盖到 service 指定的 `mental-health.jar` 后 `systemctl restart` |
| SSE 流式回复白屏、整段才返回 | Nginx 开启了代理缓冲 | `/api/` 增加 `proxy_buffering off; proxy_cache off; chunked_transfer_encoding on;` |

---

## 👨‍💻 11. 开发协作规范 (Development & Git Workflow)

为了保障项目多人协作下的代码整洁度与历史提交可追溯性，必须强制遵循以下规范：

### 11.1 Git Commit 规范 (Conventional Commits)
...
* [x] **第三阶段：实时社交全覆盖与多端泛化 (Phase 3)**
  * 强化自研 WebSocket 即时通信网关，增加 ACK 消息可靠送达验证与高并发下离线红点通知推送。
  * 完成基于 **HarmonyOS 鸿蒙 ArkTS（Stage 模型）** 的原生移动端 App，与网页端三端同源；并对前端进行渐进式网络应用 (PWA) 探索。
